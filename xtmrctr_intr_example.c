...
void enalbe_gpio_counter()
{
	XGpio_DiscreteWrite(&Gpio, LED_CHANNEL, LED);
}

void gpio_isr(uint32_t *pDev)
{
	static uint32_t gpio_isr_cnt = 0;
...
void gpioCounter_SetupIntrSystem(INTC *IntcInstancePtr)
{
	s32 Status;
	//init custom ISR
	XScuGic_SetPriorityTriggerType(IntcInstancePtr, XPAR_FABRIC_CPU_INT_IN_INTR,
									0xA0, 0x3);
	Status = XScuGic_Connect(IntcInstancePtr, XPAR_FABRIC_CPU_INT_IN_INTR,
	...
	XScuGic_Enable(IntcInstancePtr, XPAR_FABRIC_CPU_INT_IN_INTR);
}
				 ...
				 u8 TmrCtrNumber)
{
	...
	//init custom ISR
	gpioCounter_SetupIntrSystem(IntcInstancePtr);
